...
def split_image_into_balls2(image_path, output_dir, rows=9, cols=10):
    """
    Divise une image contenant une grille de boules en plusieurs images individuelles.
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # Nommer et sauvegarder l'image de la boule
            ball_filename = os.path.join(output_dir, f"boule_tourner_{row * cols + col + 1}.jpg")
            cv2.imwrite(ball_filename, ball)
...
split_image_into_balls(
    image_path="imageNormal.png",  # Chemin vers votre image de la grille
    output_dir="boules",            # Dossier de sortie
    rows=9,                         # Nombre de lignes
    cols=10                         # Nombre de colonnes
)
split_image_into_balls2(
    image_path="imageTourner.png",  # Chemin vers votre image de la grille
    output_dir="boules",            # Dossier de sortie
    rows=9,                         # Nombre de lignes
    cols=10                         # Nombre de colonnes
)
